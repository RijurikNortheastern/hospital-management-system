-- Report 3: Bed Occupancy Report
-- Shows current bed status across all rooms
...
FROM ROOM r
JOIN BED b ON r.room_id = b.ROOM_room_id
LEFT JOIN ADMISSION a ON b.bed_id = a.BED_bed_id AND a.status = 'ACTIVE'
LEFT JOIN PATIENT p ON a.PATIENT_patient_id = p.patient_id
LEFT JOIN EMPLOYEE e ON a.EMPLOYEE_employee_id = e.employee_id
